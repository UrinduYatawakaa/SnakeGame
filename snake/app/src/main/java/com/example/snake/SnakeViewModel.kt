...
class SnakeViewModel : ViewModel() {
    ...
    private val _state = MutableStateFlow(SnakeState()) //reading and writing
    val state = _state.asStateFlow() //reading only
            ...
            SnakeEvent.StartGame -> {
                ...
                viewModelScope.launch {
                    while (state.value.gameState == GameState.STARTED) {
                        val delayMillis = when(state.value.snake.size) { //speed control of snake
                            in 0..5 -> 140L
                            in 6..10 -> 110L
                            else -> 100L
                        }
    ...
    private fun updateGame(currentGame: SnakeState) : SnakeState {
        ...
        val head = currentGame.snake.first() //list fist index is head
        ...
        val newFood = if (newHead == currentGame.food) SnakeState.generateRandomFoodCoordinate() //if eats
        else currentGame.food

        //upgrade snake lenght
        if (newHead != currentGame.food) {
            newSnake = newSnake.toMutableList()
            newSnake.removeAt(newSnake.size - 1) //if not eats remove 1 else automatically adding 1
